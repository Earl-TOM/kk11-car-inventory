...
export default function AdminPasswordResetManager() {
  ...
  const approveAndGeneratePassword = async (item: PasswordResetRequest) => {
    setWorkingId(item.id);
    const updated = await passwordResetService.updateRequest(item.id, {
      status: "approved",
      generateTemporaryPassword: true,
      adminNote: "Temporary password generated by admin.",
    });
    setWorkingId(null);
    await loadItems();
    toast.success(`Temporary password: ${updated.temporaryPassword || "(not generated)"}`);
  };
  ...
  return (
    <section className="border-2 border-art-black bg-white p-6 brutalist-shadow">
      ...
      <p className="mb-4 border border-art-black/20 bg-art-beige px-3 py-2 font-serif text-sm text-art-black/80">
        Users submit reset requests here; admins generate temporary passwords and handle reset operations.
      </p>
          ...
          {items.map((item) => (
            <div key={item.id} className="border-2 border-art-black/20 p-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  ...
                  {item.temporaryPassword ? (
                    <p className="font-mono text-[10px] uppercase tracking-wider text-art-orange">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => approveAndGeneratePassword(item)}
